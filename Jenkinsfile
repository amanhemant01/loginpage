        ...
        stage ('Deploy'){
            steps{
                withCredentials([usernamePassword(credentialsId: 'Hemant_Nexus_Cred', passwordVariable: 'PASS1', usernameVariable: 'USER1'),
                    ...
                    sh label: '', script: 'curl -u $USER1:$PASS1 http://3.14.251.87:8081/nexus/content/repositories/devopstraining/Hemant/release?file=loginpage-${BUILD_NUMBER}.war -u $USER2:$PASS2 -o http://ec2-18-224-182-74.us-east-2.compute.amazonaws.com:8080/manager/text/deploy?config=file:/var/lib/tomcat8/loginpage-${BUILD_NUMBER}.war\\&path=/login_hemant'
                }
            }
        }
    }
